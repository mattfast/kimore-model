...
class ContinuousModel(Model):
	...
	def train_lstm(self, params):
		...
		position_train_pad, special_value = self.pad(position_train)

		print("Building Model")
		position_input = keras.Input(shape=(mask_seq_len,position_len))
		lstm = layers.Masking(mask_value=special_value)
		lstm = layers.LSTM(128)(position_input)
		lstm = layers.LSTM(64)(lstm)
		lstm = keras.Model(inputs=position_input, outputs=lstm)

		dct_input = keras.Input(shape=(None,dct_len))
		combined = layers.Concatenate([lstm.output, dct_input])

		prediction = layers.Dense(1, activation="linear")(combined)
		model = keras.Model(inputs=[lstm.input,dct_input],outputs=prediction)
		model.summary()
		...
		print("Fitting Model")
		model.fit(x=[position_train_pad, dct_train], y=y_train, epochs=params['epochs_1'], batch_size=num_examples)
		keras.backend.set_value(model.optimizer.learning_rate, params['lr']/2)
	...
	def predict_lstm(self, eval_data):
		...
		position_eval_pad, special_value = self.pad(position_eval)
