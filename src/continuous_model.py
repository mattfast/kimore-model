...
fcnn_default_params_continuous = {'loss_function': 'mean_squared_error', 
                  'kernel_l2': 0.01,
                  'bias_l2': 0.01,
								  'lr': 0.00005,
								  'epochs': 500}

lstm_default_params_continuous = {'loss_function': 'mean_squared_error',
                  'kernel_l2': 0.01,
                  'bias_l2': 0.01,
								  'lr': 0.005,
                  'decay_rate': 0.5,
...
gru_default_params_continuous = {'loss_function': 'mean_squared_error',
								  'max_norm_1': 50, 
								  'max_norm_2': 10,
								  'lr': 0.005,
								  'epochs_1': 300,
...
class ContinuousModel(Model):
	...
	def normalize_labels(self, y_train):

		if self.target_score == 'cTS':
			y_train -= 25
			y_train /= 25
		elif self.target_score == 'cPO':
			y_train -= 7.5
			y_train /= 7.5
		elif self.target_score == 'cCF':
			y_train -= 17.5
			y_train /= 17.5
	...
	def preprocess_training_data_variable(self):
		...
		#y_train = self.normalize_labels(np.array(y_train, dtype='float64'))
	...
	def train_fcnn(self, params=fcnn_default_params_continuous):
	...
	def predict_fcnn(self, eval_data):
		...
		print("Scaling + post-processing predictions")
	...
	def predict_variable(self, eval_data):
		...
		print("Predicting Quality Score(s)")
		predictions = self.model.predict([position_eval_pad, dct_eval])
